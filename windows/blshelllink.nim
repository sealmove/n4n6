...
# 2.1 ShellLinkHeader
struct(*shellLinkHeader, endian = l):
...
struct(*linkTargetIdList, endian = l):
  u16: *listSize
  *idList: *list(listSize)

struct(*volumeIdData, endian = l):
...
# 2.3 LinkInfo
struct(*linkInfoHeader, endian = l, size: uint32, headerSize: uint32):
...
struct(*linkInfoData, endian = l, size: uint32):
...
struct(*linkInfo, endian = l):
  u32: *size
  *linkInfoData(size): *data(size - 4)

# 2.4 StringData
struct(*stringData, endian = l):
  u16: *countCharacters
